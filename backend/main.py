...
class GrupoMensaje(Base):
    ...
    curso_id = Column(Text, nullable=False)
    curso_nombre = Column(Text, nullable=False)
    remitente_nombre = Column(Text, nullable=False)
    es_profe = Column(Boolean, default=False)
...
def subir_pdf_r2(pdf_bytes: bytes, nombre_archivo: str) -> str:
    try:
        ...
        client.put_object(Bucket=R2_BUCKET_NAME, Key=nombre_archivo, Body=pdf_bytes, ContentType="application/pdf")
...
async def send_message(token, chat_id, text, reply_markup=None) -> int:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            ...
            data = resp.json()
            if not data.get("ok"):
                print(f"[send_message] ERROR Telegram: {data}")
                return 0
...
async def send_voice(token, chat_id, file_id, source_token, caption=""):
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            r = await client.get(f"https://api.telegram.org/bot{source_token}/getFile", params={"file_id": file_id})
...
async def reenviar_archivo(token_destino, chat_id_destino, file_id, token_origen, file_name="archivo", caption="", tipo="document"):
    try:
        async with httpx.AsyncClient(timeout=120) as client:
            r = await client.get(f"https://api.telegram.org/bot{token_origen}/getFile", params={"file_id": file_id})
            file_path = r.json()["result"]["file_path"]
...
async def delete_message(token, chat_id, message_id):
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            resp = await client.post(f"https://api.telegram.org/bot{token}/deleteMessage",
                json={"chat_id": chat_id, "message_id": message_id})
            data = resp.json()
            if not data.get("ok"):
                print(f"[delete_message] No se pudo borrar {message_id} en {chat_id}: {data.get('description')}")
    except Exception as e:
        print(f"[delete_message] EXCEPTION borrando {message_id}: {e}")
 
# ── REGISTRO DE message_ids DEL GRUPO POR USUARIO ────────────────────────────
# Cada usuario (profe o estudiante) tiene su propia lista de message_ids
# correspondientes a TODO lo que ve en su chat relacionado con el grupo,
# incluyendo sus propios mensajes/archivos enviados.
...
def registrar_msg_grupo(db, telegram_id, curso_id, message_id):
    ...
    if r and r.quiz_nombre:
        try:
            ...
            ids_actuales = []
    if message_id not in ids_actuales:
...
async def borrar_mensajes_grupo_usuario(db, token, telegram_id, curso_id):
    """Borra del chat TODOS los mensajes del grupo visibles para el usuario,
    incluidos los que él mismo envió mientras estaba dentro."""
    msg_ids = obtener_msg_ids_grupo(db, telegram_id, curso_id)
    print(f"[SALIR_GRUPO] Borrando {len(msg_ids)} mensajes para {telegram_id} en curso {curso_id}")
    for mid in msg_ids:
        await delete_message(token, telegram_id, mid)
    limpiar_msg_ids_grupo(db, telegram_id, curso_id)
 
# ── CHAT INDIVIDUAL PROFE <-> ESTUDIANTE (dentro de un grupo) ────────────────
# Permite al profe, estando inmerso en un grupo, abrir un chat 1-a-1 con un
# estudiante sin salir del grupo y sin que el resto vea nada. Se pausa la
...
def salir_chat_individual(db, profe_telegram_id):
    db.query(Resultado).filter(
        Resultado.nombre_temp == f"__chat_individual__{profe_telegram_id}"
    ).delete(synchronize_session=False)
    db.commit()
 
def registrar_msg_chat_individual(db, telegram_id, otro_id, message_id):
    """Registra message_ids del chat individual, separado del historial del grupo."""
    if not message_id:
        return
    clave = f"__chat_ind_msg__{telegram_id}__{otro_id}"
    r = db.query(Resultado).filter(Resultado.nombre_temp == clave).first()
    ids_actuales = []
    if r and r.quiz_nombre:
        try:
...
async def borrar_chat_individual(db, token_propio, telegram_id_propio, token_otro, telegram_id_otro):
    """Borra del chat de AMBOS lados (profe y estudiante) los mensajes del chat individual."""
    clave_propia = f"__chat_ind_msg__{telegram_id_propio}__{telegram_id_otro}"
    clave_otro = f"__chat_ind_msg__{telegram_id_otro}__{telegram_id_propio}"
 
    r_propia = db.query(Resultado).filter(Resultado.nombre_temp == clave_propia).first()
    ids_propios = json.loads(r_propia.quiz_nombre) if r_propia and r_propia.quiz_nombre else []
    for mid in ids_propios:
        await delete_message(token_propio, telegram_id_propio, mid)
    db.query(Resultado).filter(Resultado.nombre_temp == clave_propia).delete(synchronize_session=False)
 
    r_otro = db.query(Resultado).filter(Resultado.nombre_temp == clave_otro).first()
    ids_otro = json.loads(r_otro.quiz_nombre) if r_otro and r_otro.quiz_nombre else []
    for mid in ids_otro:
        await delete_message(token_otro, telegram_id_otro, mid)
    db.query(Resultado).filter(Resultado.nombre_temp == clave_otro).delete(synchronize_session=False)
 
    db.commit()
 
async def enviar_chat_individual(db, remitente_tid, destinatario_tid, tipo, file_id=None, text=None, file_name="archivo", remitente_es_profe=True):
    """Envía un mensaje/archivo de un lado al otro del chat individual (profe<->estudiante),
    y registra los message_id del lado RECEPTOR para poder limpiarlos después.
    remitente_es_profe=True  -> el profe le escribe al estudiante (token origen: BOT_PROFE_TOKEN, destino: BOT_ESTUDIANTE_TOKEN)
    remitente_es_profe=False -> el estudiante le escribe al profe (token origen: BOT_ESTUDIANTE_TOKEN, destino: BOT_PROFE_TOKEN)
    """
    if remitente_es_profe:
        token_origen = BOT_PROFE_TOKEN
        token_destino = BOT_ESTUDIANTE_TOKEN
        prefijo = "💬 <b>[Privado] Tu profe:</b>"
    else:
        token_origen = BOT_ESTUDIANTE_TOKEN
        token_destino = BOT_PROFE_TOKEN
        est_obj_tmp = db.query(Estudiante).filter(Estudiante.telegram_id == remitente_tid).first()
        nombre_est_tmp = est_obj_tmp.nombre if est_obj_tmp else "Un estudiante"
        prefijo = f"💬 <b>[Privado] {nombre_est_tmp}:</b>"
 
    async with httpx.AsyncClient(timeout=60) as client:
        if tipo == "text":
            resp = await client.post(f"https://api.telegram.org/bot{token_destino}/sendMessage",
                json={"chat_id": destinatario_tid, "text": f"{prefijo}\n{text}", "parse_mode": "HTML"})
            data = resp.json()
            if data.get("ok"):
                registrar_msg_chat_individual(db, destinatario_tid, remitente_tid, data["result"]["message_id"])
        elif tipo == "voice":
            r = await client.get(f"https://api.telegram.org/bot{token_origen}/getFile", params={"file_id": file_id})
            fp = r.json()["result"]["file_path"]
            audio_r = await client.get(f"https://api.telegram.org/file/bot{token_origen}/{fp}")
            resp = await client.post(f"https://api.telegram.org/bot{token_destino}/sendVoice",
                data={"chat_id": destinatario_tid, "caption": prefijo, "parse_mode": "HTML"},
                files={"voice": ("voice.ogg", audio_r.content, "audio/ogg")})
            data = resp.json()
            if data.get("ok"):
                registrar_msg_chat_individual(db, destinatario_tid, remitente_tid, data["result"]["message_id"])
            ...
            r = await client.get(f"https://api.telegram.org/bot{token_origen}/getFile", params={"file_id": file_id})
            fp = r.json()["result"]["file_path"]
            file_r = await client.get(f"https://api.telegram.org/file/bot{token_origen}/{fp}")
            resp = await client.post(f"https://api.telegram.org/bot{token_destino}/sendPhoto",
                data={"chat_id": destinatario_tid, "caption": prefijo, "parse_mode": "HTML"},
            ...
            fp = r.json()["result"]["file_path"]
            file_r = await client.get(f"https://api.telegram.org/file/bot{token_origen}/{fp}")
            resp = await client.post(f"https://api.telegram.org/bot{token_destino}/sendVideo",
                data={"chat_id": destinatario_tid, "caption": prefijo, "parse_mode": "HTML"},
            ...
            fp = r.json()["result"]["file_path"]
            file_r = await client.get(f"https://api.telegram.org/file/bot{token_origen}/{fp}")
            resp = await client.post(f"https://api.telegram.org/bot{token_destino}/sendDocument",
                data={"chat_id": destinatario_tid, "caption": f"{prefijo} 📎 {file_name}", "parse_mode": "HTML"},
...
def get_estudiantes_telegram_ids(db, curso_nombre, profe_telegram_id):
    ...
    for (nombre_r,) in nombres:
        if not nombre_r:
            continue
        est = db.query(Estudiante).filter(Estudiante.nombre.ilike(f"%{nombre_r}%")).first()
        if not est:
            for parte in nombre_r.strip().split():
                if len(parte) > 2:
                    est = db.query(Estudiante).filter(Estudiante.nombre.ilike(f"%{parte}%")).first()
                    if est:
                        break
...
async def enviar_media_grupo_con_registro(client, token, chat_id, origen_token, tipo, file_id, file_name, prefijo, db, curso_id):
    """Envía archivo al grupo y registra su message_id. Retorna True si exitoso."""
    msg_id = 0
    exito = False
    try:
        if tipo == "photo":
            resp = await client.post(f"https://api.telegram.org/bot{token}/sendPhoto",
                json={"chat_id": chat_id, "photo": file_id, "caption": prefijo, "parse_mode": "HTML"})
            data = resp.json()
            if not data.get("ok"):
            ...
            if data.get("ok"):
                ...
                print(f"[media:photo] FALLO enviando a {chat_id}: {data}")
            ...
            if data.get("ok"):
                ...
                print(f"[media:document] FALLO enviando a {chat_id}: {data}")
            ...
            if data.get("ok"):
                ...
                print(f"[media:voice] FALLO enviando a {chat_id}: {data}")
 
    except Exception as e:
        print(f"[enviar_media_grupo_con_registro] EXCEPTION tipo={tipo} chat_id={chat_id}: {e}")
...
async def transmitir_grupo(db, curso_id, remitente_id, remitente_nombre, es_profe,
                            ...
                            remitente_msg_id=None):
    """
    Transmite un mensaje/archivo a todos los miembros del grupo (excepto el remitente).
    También registra el message_id ORIGINAL del remitente en su propia lista,
    para poder borrarlo cuando él mismo salga del grupo.
    """
    ...
    # Guardar en historial persistente (BD) — esto NUNCA se borra
    ...
    # Registrar el mensaje ORIGINAL del remitente en su propia lista de mensajes
    # visibles, para poder borrarlo cuando él mismo salga del grupo.
    if remitente_msg_id:
        registrar_msg_grupo(db, remitente_id, str(curso_id), remitente_msg_id)
 
    # Destinatarios: todos los estudiantes del curso + el profe, EXCEPTO el remitente
    destinatarios = []
    for tid in est_ids:
        if tid != remitente_id:
    ...
    print(f"[transmitir_grupo] curso='{curso.nombre}' remitente_id={remitente_id} es_profe={es_profe} tipo={tipo} | destinatarios={len(destinatarios)} -> {destinatarios}")
 
    if not destinatarios:
        print(f"[transmitir_grupo] Sin destinatarios para este envío.")
        return
            ...
            try:
                if tipo == "text":
                    ...
                    if data.get("ok"):
                        ...
                        print(f"[transmitir_grupo] OK texto -> {chat_id} (msg_id={mid})")
                    else:
                        print(f"[transmitir_grupo] FALLO texto -> {chat_id}: {data}")
                else:
                    ok = await enviar_media_grupo_con_registro(client, token, chat_id, origen_token, tipo, file_id, file_name, prefijo, db, curso_id)
                    print(f"[transmitir_grupo] media tipo={tipo} -> {chat_id}: {'OK' if ok else 'FALLO'}")
            except Exception as e:
...
async def mostrar_historial_grupo(db, token, chat_id, curso_id, desde_cuando=None):
    ...
    for m in mensajes:
        ...
        try:
                ...
                if m.tipo == "text" and m.contenido:
                    ...
                    if not data.get("ok"):
                        ...
                        if r.json().get("ok"):
                            ...
                            data = resp.json()
                    if data.get("ok"):
                        registrar_msg_grupo(db, chat_id, str(curso_id), data["result"]["message_id"])
            ...
            print(f"[mostrar_historial_grupo] EXCEPTION mostrando {m.tipo}: {e}")
...
async def webhook_profe(request: Request, db: Session = Depends(get_db)):
        ...
        if cb_data.startswith("grupo_profe_"):
            ...
            if curso:
                ...
                mid = await send_message(BOT_PROFE_TOKEN, chat_id,
                    ...
                    f"Usa /miembros para ver la lista y chatear en privado con un estudiante.\n"
                    f"Usa /salir_grupo para volver a tu chat normal.")
                registrar_msg_grupo(db, chat_id, curso_id, mid)
                await mostrar_historial_grupo(db, BOT_PROFE_TOKEN, chat_id, curso_id)
                ...
                est_ids = get_estudiantes_telegram_ids(db, curso.nombre, profe_tid)
                for est_tid in est_ids:
                    try:
                        await send_message(BOT_ESTUDIANTE_TOKEN, est_tid,
                            f"🔔 <b>El profe {profe_nombre} se unió al grupo de {curso.nombre} {curso.grado}</b>\n\n"
                            f"Usa /grupos para entrar y ver lo que comparte.")
                    except:
                        pass
 
        elif cb_data.startswith("chat_ind_"):
            resto = cb_data.replace("chat_ind_", "")
            try:
                est_tid_str, curso_id_ind = resto.split("_", 1)
                est_tid_ind = int(est_tid_str)
            except:
                await send_message(BOT_PROFE_TOKEN, chat_id, "❌ Error procesando la selección.")
                return {"ok": True}
            est_obj = db.query(Estudiante).filter(Estudiante.telegram_id == est_tid_ind).first()
            nombre_est_ind = est_obj.nombre if est_obj else str(est_tid_ind)
            # Si el estudiante YA inició este chat individual (vía /enviar_profesor),
            # no hace falta volver a avisarle ni reiniciar su sesión: solo el profe entra.
            chat_ind_existente_est = get_chat_individual(db, est_tid_ind)
            ya_iniciado_por_estudiante = bool(
                chat_ind_existente_est and chat_ind_existente_est[0] == telegram_id
            )
            entrar_chat_individual(db, telegram_id, est_tid_ind, curso_id_ind)
            mid = await send_message(BOT_PROFE_TOKEN, chat_id,
                f"💬 <b>Chat privado con {nombre_est_ind}</b>\n\n"
                f"Todo lo que escribas o envíes (texto, imágenes, PDF, Word, otros archivos) "
                f"llegará solo a este estudiante.\n\n"
                f"Usa /volver_grupo para regresar al grupo.")
            registrar_msg_chat_individual(db, telegram_id, est_tid_ind, mid)
            if not ya_iniciado_por_estudiante:
                mid_est = await send_message(BOT_ESTUDIANTE_TOKEN, est_tid_ind,
                    f"💬 <b>Tu profe abrió un chat privado contigo</b>\n\n"
                    f"Esta conversación es solo entre tú y tu profe, no la ven tus compañeros.")
                registrar_msg_chat_individual(db, est_tid_ind, telegram_id, mid_est)
    ...
    if chat_ind and text != "/volver_grupo":
        ...
        if voice:
            ...
            return {"ok": True}
        if photo:
            await enviar_chat_individual(db, telegram_id, estudiante_tid, "photo", file_id=photo[-1].get("file_id"), file_name="imagen.jpg")
            return {"ok": True}
        if video:
            fname = video.get("file_name", "video.mp4")
            await enviar_chat_individual(db, telegram_id, estudiante_tid, "video", file_id=video.get("file_id"), file_name=fname)
            return {"ok": True}
        if document:
            fname = document.get("file_name", "archivo")
            await enviar_chat_individual(db, telegram_id, estudiante_tid, "document", file_id=document.get("file_id"), file_name=fname)
            return {"ok": True}
        if text and not text.startswith("/"):
            await enviar_chat_individual(db, telegram_id, estudiante_tid, "text", text=text)
            return {"ok": True}
        ...
        if voice:
            ...
            if paso and paso.startswith("responder_voz_"):
                try:
                    ...
                    await send_voice(BOT_ESTUDIANTE_TOKEN, estudiante_dest, voice_file_id, BOT_PROFE_TOKEN, "🎙️ Nota de voz de tu profe")
                ...
                await transmitir_grupo(db, grupo_activo, telegram_id, nombre, True, "voice",
                    file_id=voice_file_id, remitente_msg_id=incoming_msg_id)
            return {"ok": True}
        if photo:
            fid = photo[-1].get("file_id")
            await transmitir_grupo(db, grupo_activo, telegram_id, nombre, True, "photo",
                file_id=fid, file_name="imagen.jpg", remitente_msg_id=incoming_msg_id)
            return {"ok": True}
        if video:
            fid = video.get("file_id")
            fname = video.get("file_name", "video.mp4")
            await transmitir_grupo(db, grupo_activo, telegram_id, nombre, True, "video",
    ...
    if archivo_recibido:
        ...
        if paso == "esperando_archivo_para_curso":
            ...
            if curso_info:
                curso_id, curso_nombre_env = curso_info.split("|", 1)
                del_estado(db, telegram_id, "paso")
                del_estado(db, telegram_id, "enviar_archivo_curso")
                curso_obj = db.query(Curso).filter(Curso.id == curso_id).first()
                profe_obj = db.query(Profe).filter(Profe.id == curso_obj.profe_id).first() if curso_obj else None
                profe_tid = profe_obj.telegram_id if profe_obj else None
                est_ids = get_estudiantes_telegram_ids(db, curso_nombre_env, profe_tid)
                if not est_ids:
                    await send_message(BOT_PROFE_TOKEN, chat_id,
                        f"⚠️ No hay estudiantes registrados en <b>{curso_nombre_env}</b>.\n\nLos estudiantes deben haber iniciado el bot y escrito su nombre.")
                else:
                    await send_message(BOT_PROFE_TOKEN, chat_id, f"⏳ Enviando a {len(est_ids)} estudiantes...")
                    enviados = 0
                    for est_tid in est_ids:
                        try:
                            await reenviar_archivo(BOT_ESTUDIANTE_TOKEN, est_tid, archivo_recibido, BOT_PROFE_TOKEN,
                                archivo_nombre, f"📎 Archivo de tu profe — {curso_nombre_env}", archivo_tipo)
                            enviados += 1
                        except:
                            pass
                    await send_message(BOT_PROFE_TOKEN, chat_id,
            ...
            del_estado(db, telegram_id, "paso")
            try:
                await reenviar_archivo(BOT_ESTUDIANTE_TOKEN, estudiante_dest, archivo_recibido, BOT_PROFE_TOKEN,
                    archivo_nombre, "📎 Archivo de tu profe", archivo_tipo)
            ...
            if resultados_pendientes and paso_actual == "esperando_pdf_quiz":
                ...
                try:
                    ...
                    for r in resultados_lista:
                        nuevo_r = Resultado(
                            id=uuid.uuid4(), nombre_temp=r["nombre"], nota=r["nota"],
                            puntos=r["puntos"], posibles=r["posibles"], porcentaje=r["porcentaje"],
                            pagina=r.get("pagina", 0), imagen_url=r.get("imagen_url", ""),
                            curso_nombre=curso_nombre, quiz_nombre=qnombre,
                            profe_telegram_id=telegram_id, confirmado=False
                        )
                        db.add(nuevo_r)
                    db.commit()
                    ...
                    await send_message(BOT_PROFE_TOKEN, chat_id,
                        f"✅ PDF procesado: <b>{total} estudiantes</b>\n\n{resumen}\n\n📄 Ahora envíame el PDF del quiz (las preguntas).")
                except Exception as e:
            ...
            if cursos:
                ...
                await send_message(BOT_PROFE_TOKEN, chat_id, "📤 ¿A qué curso quieres enviar este archivo?", reply_markup=botones)
            else:
    ...
    if text == "/start":
        ...
        grupo_activo = get_grupo_activo(db, telegram_id)
        if not grupo_activo:
            await send_message(BOT_PROFE_TOKEN, chat_id, "❌ Primero entra a un grupo con /grupos.")
            return {"ok": True}
        curso = db.query(Curso).filter(Curso.id == grupo_activo).first()
        if not curso:
            await send_message(BOT_PROFE_TOKEN, chat_id, "❌ No encontré el curso del grupo actual.")
            return {"ok": True}
        profe_obj = db.query(Profe).filter(Profe.telegram_id == telegram_id).first()
        profe_tid = profe_obj.telegram_id if profe_obj else telegram_id
        est_ids = get_estudiantes_telegram_ids(db, curso.nombre, profe_tid)
        if not est_ids:
            await send_message(BOT_PROFE_TOKEN, chat_id, "❌ No hay estudiantes registrados en este grupo todavía.")
            return {"ok": True}
        botones_lista = []
        for est_tid in est_ids:
            est_obj = db.query(Estudiante).filter(Estudiante.telegram_id == est_tid).first()
            nombre_est = est_obj.nombre if est_obj else str(est_tid)
            botones_lista.append([{"text": f"👤 {nombre_est}", "callback_data": f"chat_ind_{est_tid}_{grupo_activo}"}])
        await send_message(BOT_PROFE_TOKEN, chat_id,
            f"👥 <b>Miembros del grupo {curso.nombre} {curso.grado}</b>\n\nSelecciona un estudiante para chatear en privado:",
        ...
        if len(partes) >= 2:
            try:
                ...
                await send_message(BOT_PROFE_TOKEN, chat_id, f"📤 Listo. Ahora envía el archivo para el estudiante <code>{estudiante_id}</code>.")
            except:
        ...
        if len(partes) >= 2:
            try:
                ...
                await send_message(BOT_PROFE_TOKEN, chat_id, f"🎙️ Listo. Ahora graba y envía tu nota de voz para el estudiante <code>{estudiante_id}</code>.")
            except:
        ...
        if len(partes) >= 3:
            try:
                ...
                respuesta = partes[2]
                await send_message(BOT_ESTUDIANTE_TOKEN, estudiante_chat_id, f"📬 <b>Respuesta de tu profe:</b>\n\n{respuesta}")
        ...
        if paso == "esperando_nombre_curso" and profe and profe.activo:
            ...
            if not resultados_db:
                await send_message(BOT_PROFE_TOKEN, chat_id, "❌ No encontré el PDF procesado. Por favor vuelve a enviar el PDF primero.")
                return {"ok": True}
...
async def webhook_estudiante(request: Request, db: Session = Depends(get_db)):
        ...
        if cb_data.startswith("grupo_est_"):
            ...
            if curso:
                ...
                mid = await send_message(BOT_ESTUDIANTE_TOKEN, chat_id,
                    ...
                    f"Usa /enviar_profesor si quieres escribirle algo en privado solo a tu profe.\n"
                    f"Usa /salir_grupo para volver a tu chat personal.")
                registrar_msg_grupo(db, chat_id, curso_id, mid)
                await mostrar_historial_grupo(db, BOT_ESTUDIANTE_TOKEN, chat_id, curso_id)
                profe_curso = db.query(Profe).filter(Profe.id == curso.profe_id).first()
                if profe_curso:
                    try:
    ...
    # ── CHAT INDIVIDUAL ACTIVO (estudiante hablando en privado con el profe) ─
    ...
    if chat_ind_est and text != "/volver_grupo":
        ...
        if voice:
            await enviar_chat_individual(db, telegram_id, profe_tid_ind, "voice", file_id=voice.get("file_id"), remitente_es_profe=False)
            return {"ok": True}
        if photo:
            await enviar_chat_individual(db, telegram_id, profe_tid_ind, "photo", file_id=photo[-1].get("file_id"), file_name="imagen.jpg", remitente_es_profe=False)
            return {"ok": True}
        if video:
            fname = video.get("file_name", "video.mp4")
            await enviar_chat_individual(db, telegram_id, profe_tid_ind, "video", file_id=video.get("file_id"), file_name=fname, remitente_es_profe=False)
            return {"ok": True}
        if document:
            fname = document.get("file_name", "archivo")
            await enviar_chat_individual(db, telegram_id, profe_tid_ind, "document", file_id=document.get("file_id"), file_name=fname, remitente_es_profe=False)
            return {"ok": True}
        if text and not text.startswith("/"):
            await enviar_chat_individual(db, telegram_id, profe_tid_ind, "text", text=text, remitente_es_profe=False)
            return {"ok": True}
 
    elif text == "/volver_grupo":
        if chat_ind_est:
            profe_tid_ind, curso_id_ind_est, desde_cuando_est = chat_ind_est
            await borrar_chat_individual(db, BOT_ESTUDIANTE_TOKEN, telegram_id, BOT_PROFE_TOKEN, profe_tid_ind)
            salir_chat_individual(db, telegram_id)
            curso_volver_est = db.query(Curso).filter(Curso.id == curso_id_ind_est).first()
            nombre_curso_volver_est = f"{curso_volver_est.nombre} {curso_volver_est.grado}" if curso_volver_est else "el grupo"
            mid = await send_message(BOT_ESTUDIANTE_TOKEN, chat_id,
                f"✅ <b>Volviste al grupo: {nombre_curso_volver_est}</b>")
            registrar_msg_grupo(db, chat_id, curso_id_ind_est, mid)
            await mostrar_historial_grupo(db, BOT_ESTUDIANTE_TOKEN, chat_id, curso_id_ind_est, desde_cuando=desde_cuando_est)
            return {"ok": True}
        else:
    ...
    grupo_activo = get_grupo_activo(db, telegram_id)
    if grupo_activo and text != "/salir_grupo" and text != "/grupos" and text != "/duda" and text != "/grafico" and text != "/enviar_profesor":
        ...
        if voice:
            await transmitir_grupo(db, grupo_activo, telegram_id, nombre_est, False, "voice",
                file_id=voice.get("file_id"), remitente_msg_id=incoming_msg_id)
            return {"ok": True}
        if photo:
            await transmitir_grupo(db, grupo_activo, telegram_id, nombre_est, False, "photo",
                file_id=photo[-1].get("file_id"), file_name="imagen.jpg", remitente_msg_id=incoming_msg_id)
            return {"ok": True}
        if video:
            fname = video.get("file_name", "video.mp4")
            await transmitir_grupo(db, grupo_activo, telegram_id, nombre_est, False, "video",
                file_id=video.get("file_id"), file_name=fname, remitente_msg_id=incoming_msg_id)
            return {"ok": True}
        if document:
            fname = document.get("file_name", "archivo")
            await transmitir_grupo(db, grupo_activo, telegram_id, nombre_est, False, "document",
                file_id=document.get("file_id"), file_name=fname, remitente_msg_id=incoming_msg_id)
            return {"ok": True}
        if text and not text.startswith("/"):
            await transmitir_grupo(db, grupo_activo, telegram_id, nombre_est, False, "text",
                text=text, remitente_msg_id=incoming_msg_id)
            return {"ok": True}
 
    if text == "/enviar_profesor":
        if not grupo_activo:
            await send_message(BOT_ESTUDIANTE_TOKEN, chat_id, "❌ Primero entra a un grupo con /grupos para poder escribirle al profe de esa materia.")
            return {"ok": True}
        curso_envprofe = db.query(Curso).filter(Curso.id == grupo_activo).first()
        if not curso_envprofe:
            await send_message(BOT_ESTUDIANTE_TOKEN, chat_id, "❌ No encontré el curso de tu grupo actual.")
            return {"ok": True}
        profe_envprofe = db.query(Profe).filter(Profe.id == curso_envprofe.profe_id).first()
        if not profe_envprofe:
            await send_message(BOT_ESTUDIANTE_TOKEN, chat_id, "❌ No encontré al profe de este grupo.")
            return {"ok": True}
        nombre_est_envprofe = estudiante.nombre if estudiante else nombre
        entrar_chat_individual(db, telegram_id, profe_envprofe.telegram_id, grupo_activo)
        mid = await send_message(BOT_ESTUDIANTE_TOKEN, chat_id,
            f"💬 <b>Chat privado con tu profe ({curso_envprofe.nombre} {curso_envprofe.grado})</b>\n\n"
            f"Todo lo que escribas o envíes (texto, imágenes, PDF, Word, otros archivos) "
            f"llegará solo a tu profe.\n\n"
            f"Usa /volver_grupo para regresar al grupo.")
        registrar_msg_chat_individual(db, telegram_id, profe_envprofe.telegram_id, mid)
        try:
            mid_profe = await send_message(BOT_PROFE_TOKEN, profe_envprofe.telegram_id,
                f"📩 <b>{nombre_est_envprofe}</b>, estudiante del grupo <b>{curso_envprofe.nombre} {curso_envprofe.grado}</b>, te ha enviado un mensaje.\n\n"
                f"Usa el botón para responderle en privado:",
                reply_markup={"inline_keyboard": [[{
                    "text": f"💬 Responder a {nombre_est_envprofe}",
                    "callback_data": f"chat_ind_{telegram_id}_{grupo_activo}"
                }]]})
            registrar_msg_chat_individual(db, profe_envprofe.telegram_id, telegram_id, mid_profe)
        except:
            pass
        return {"ok": True}
    ...
    if voice:
        ...
        if esperando == "si" and sesion_activa(db, telegram_id):
            ...
            await send_message(BOT_ESTUDIANTE_TOKEN, chat_id, "⏰ Tu sesion de 15 minutos ha expirado.\n\nUsa /duda para iniciar una nueva sesion.")
        else:
    ...
    if archivo_recibido:
        ...
        if esperando == "si" and sesion_activa(db, telegram_id):
            ...
            await send_message(BOT_PROFE_TOKEN, profe_dest,
                ...
                f"Para enviar archivo: <code>/enviar_a {telegram_id}</code>")
            await reenviar_archivo(BOT_PROFE_TOKEN, profe_dest, archivo_recibido, BOT_ESTUDIANTE_TOKEN, archivo_nombre, "", archivo_tipo)
    ...
    if text == "/start":
        if not estudiante:
            ...
            await send_message(BOT_ESTUDIANTE_TOKEN, chat_id,
                ...
                f"Para ver tus notas y resultados:\n"
                f"1️⃣ Escribe tu <b>nombre completo</b> tal como aparece en tu lista de clase\n"
                f"2️⃣ Luego usa /misnotas para ver todas tus notas, hojas de respuesta y quizzes\n\n"
        ...
        if not est or not est.nombre:
            await send_message(BOT_ESTUDIANTE_TOKEN, chat_id, "❌ Primero escribe tu nombre completo para registrarte.")
            return {"ok": True}
        materias = db.query(Resultado.curso_nombre, Resultado.profe_telegram_id).filter(
            Resultado.nombre_temp.ilike(f"%{est.nombre}%"),
            Resultado.confirmado == True, Resultado.curso_nombre != None, Resultado.profe_telegram_id != None
        ).distinct().all()
        if not materias:
            await send_message(BOT_ESTUDIANTE_TOKEN, chat_id, "❌ No tienes materias registradas aun. Escribe tu nombre primero.")
        else:
        ...
        if not est or not est.nombre:
            ...
            return {"ok": True}
        resultados = db.query(Resultado).filter(
            Resultado.nombre_temp.ilike(f"%{est.nombre}%"), Resultado.confirmado == True, Resultado.curso_nombre != None
        ).all()
        if not resultados:
        ...
        if not est or not est.nombre:
            ...
            return {"ok": True}
        # Buscar resultados por nombre exacto primero, luego por partes del nombre
        resultados = db.query(Resultado).filter(
            Resultado.nombre_temp.ilike(f"%{est.nombre}%"),
            Resultado.confirmado == True,
            Resultado.curso_nombre != None
        ).all()
        # Si no encuentra, intentar con partes del nombre
        if not resultados:
        ...
        if not resultados:
            await send_message(BOT_ESTUDIANTE_TOKEN, chat_id,
                ...
                f"Verifica que tu nombre esté escrito exactamente igual al de tu lista de clase.")
            return {"ok": True}
        # Agrupar por curso y quiz
        await send_message(BOT_ESTUDIANTE_TOKEN, chat_id,
            f"📊 <b>Tus resultados, {est.nombre}:</b>\n\n"
            f"A continuación te envío cada evaluación con tu nota, hoja de respuestas y el PDF del quiz.")
        for r in resultados:
            curso = r.curso_nombre or "Sin curso"
            ...
            porcentaje = float(r.porcentaje) if r.porcentaje else 0
            estado = "✅ Aprobado" if nota >= 3.0 else "❌ Reprobado"
            # Enviar resumen de la nota
            await send_message(BOT_ESTUDIANTE_TOKEN, chat_id,
                f"📚 <b>{curso}</b>\n"
                f"📝 <b>{quiz}</b>\n"
                f"🎯 Nota: <b>{nota:.2f} / 5.0</b> ({porcentaje:.0f}%)\n"
                f"{estado}")
            # Enviar imagen de hoja de respuestas si existe
            if r.imagen_url:
                await send_photo(BOT_ESTUDIANTE_TOKEN, chat_id, r.imagen_url,
                    f"📋 Tu hoja de respuestas — {quiz}")
            # Enviar PDF del quiz si existe
            if r.quiz_pdf_url:
        ...
        if esperando == "si" and sesion_activa(db, telegram_id):
            ...
            await send_message(BOT_PROFE_TOKEN, profe_dest,
                ...
                f"Para responder con voz: <code>/responder_voz {telegram_id}</code>\n"
                f"Para enviar archivo: <code>/enviar_a {telegram_id}</code>")
            await send_message(BOT_ESTUDIANTE_TOKEN, chat_id, f"✅ Mensaje enviado. Sesion activa por {mins} min mas.")
        elif esperando == "si" and not sesion_activa(db, telegram_id):
            del_estado(db, telegram_id, "esperando_duda")
            del_estado(db, telegram_id, "duda_materia")
            del_estado(db, telegram_id, "duda_profe_id")
            ...
            busqueda = text.strip()
            resultados_materia = db.query(Resultado).filter(Resultado.curso_nombre.ilike(f"%{busqueda}%"), Resultado.confirmado == True).all()
            resultados_nombre = db.query(Resultado).filter(Resultado.nombre_temp.ilike(f"%{busqueda}%"), Resultado.confirmado == True).all()
            if resultados_materia and not resultados_nombre:
                ...
                if est and est.nombre:
                    ...
                    if not resultados:
                        await send_message(BOT_ESTUDIANTE_TOKEN, chat_id, f"❌ No encontré tus notas en <b>{busqueda}</b>.")
                    else:
                        ...
                        for r in resultados:
                            if r.imagen_url:
                                await send_photo(BOT_ESTUDIANTE_TOKEN, chat_id, r.imagen_url, f"📋 {r.quiz_nombre} - Tu hoja de respuestas")
                            if r.quiz_pdf_url:
                                await send_document_url(BOT_ESTUDIANTE_TOKEN, chat_id, r.quiz_pdf_url, f"📄 {r.quiz_nombre} - PDF del quiz")
                else:
                    await send_message(BOT_ESTUDIANTE_TOKEN, chat_id, f"❌ Primero escribe tu nombre completo para registrarte.")
